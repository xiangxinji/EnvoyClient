...
export const resourceService = defineService({
  name: "resource",
  operations: [
    {
      ...
      parameters: {
        type: "object",
        properties: { path: { type: "string", description: "本地文件路径（~ 开头表示用户 home 目录）" } },
        required: ["path"],
      },
      run: async (args, ctx) => {
        if (!isTauri) return { error: "Not in Tauri environment" };
        if (!ctx.taskId) return { error: "No task ID in context" };
    ...
    },
    {
      ...
      parameters: {
        type: "object",
        properties: { name: { type: "string", description: "技能名称（文件名，不含 .md 后缀）" } },
        required: ["name"],
      },
      run: async (args, ctx) => {
        if (!isTauri) return { error: "Not in Tauri environment" };
        const name = args.name as string;
        return invoke("file_read", { path: `~/.envoy/brains/${ctx.myId}/skills/${name}.md` });
      },
    },
  ],
});
