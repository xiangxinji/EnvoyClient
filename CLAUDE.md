...
| 子组件名 | 用途 | 去向 |
|---|---|---|
| `TaskActionButtons` | 开始/上传/完成/审批按钮组 | `src/components/TaskActionButtons/` |
| `TaskFileList` | 文件资源列表（文件名+大小+下载） | `src/components/TaskFileList/` |
| `TaskReviewList` | Leader 评审结果列表 | `src/components/TaskReviewList/` |
| `TaskTraceBlock` | 执行步骤/工具调用嵌套渲染 | `src/components/TaskTraceBlock/` |

### 待提取的 Composable

#### ~~`useTaskLiveData(taskId)` — ✅ 已完成~~

已提取为 `src/composables/useTaskLiveData.ts`。

#### ~~`useTaskTimeline(resources, timestamp)` — ✅ 已完成~~

已提取为 `src/composables/useTaskTimeline.ts`，包含 `TimelineEvent` 接口和 `timelineEvents`/`duration` 计算属性。

#### ~~`useCloudFiles(teamName, myId)` — ✅ 已完成~~

已提取为 `src/composables/useCloudFiles.ts`。
...
已提取为 `src/composables/useChatSend.ts`。

#### ~~`useKnowledgeSettings` — ✅ 已完成~~

已从 `SettingsKnowledge/main.vue` 提取为 `src/composables/useKnowledgeSettings.ts`。

#### ~~`useTaskCenterData` — ✅ 已完成~~

已从 `TaskCenterView/main.vue` 提取为 `src/composables/useTaskCenterData.ts`。

#### ~~`useAuth` — ✅ 已完成~~

已从 `RoleSelect/main.vue` 提取为 `src/composables/useAuth.ts`。

#### ~~`useTeamClientNotifications` + `useTeamClientMessageRouter` — ✅ 已完成~~

已从 `useTeamClient.ts` 提取为 `src/composables/useTeamClientNotifications.ts` 和 `src/composables/useTeamClientMessageRouter.ts`。

#### ~~`useChatNewMessageTracker` — ✅ 已完成~~

已从 `ChatPanel/main.vue` 提取为 `src/composables/useChatNewMessageTracker.ts`。

### 共享逻辑统一

#### ~~AutoReply 切换逻辑 — ✅ 已完成~~

已在 `useMemberSettings` 中添加统一的 `toggleAutoReply` 方法。

#### ~~Peer ID 解析 — ✅ 已完成~~

已提取为 `resolvePeerId` 工具函数到 `src/utils/chatMessageFactory.ts`。

#### ~~ChatMessage 构造 — ✅ 已完成~~

已提取为 `buildChatMessage` 工厂函数到 `src/utils/chatMessageFactory.ts`。

### 状态管理规范

#### ~~禁止设置面板的影子状态 — ✅ 已完成（SettingsAI）~~

`SettingsAI/main.vue` 已改用 computed getter/setter 绑定到 `settings.value`，消除 shadow state。

```ts
// 正确做法
const aiAutoReply = computed({
  get: () => settings.value.ai_auto_reply,
  set: (val) => saveSettings(myId, { ai_auto_reply: val }),
});
```
...
#### ~~CloudResourcesPanel 必须使用 useConfirm — ✅ 已完成~~

`CloudResourcesPanel/main.vue` 已通过 `useCloudFiles` composable 使用 `useConfirm`，不再手写确认状态。
...
`useFullscreenViewer.ts` 已改为模块级单例模式（refs + functions 定义在模块作用域），所有调用方共享同一实例。

### ~~document 级事件监听器规范~~ — ✅ 已确认无此问题

`MessageBubble` 和 `BubbleContent` 均未使用 `document.addEventListener`。`useFullscreenViewer` 的 keydown 监听器已通过 singleton watcher 正确管理生命周期。
