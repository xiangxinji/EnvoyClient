...
系统 SHALL 在应用启动时显示角色选择页面，包含 Leader 和 Member 两个选项，以及 Client ID 输入框和 Server URL 输入框。所有 UI 文本（标签、按钮、占位符、标题、副标题）SHALL 通过 `t()` 函数获取，支持多语言切换。
...
#### Scenario: 英文界面下表单显示
- **WHEN** 当前语言为英文
- **THEN** 表单中所有标签、按钮文字、占位符显示英文文本

### Requirement: Connection validation
系统 SHALL 验证 Client ID 非空且 Server URL 格式合法（ws:// 或 wss:// 开头），验证失败时在对应输入框下方显示错误提示。错误提示文本 SHALL 通过 `t()` 函数获取。

#### Scenario: Empty client ID
- **WHEN** 用户未填写 Client ID 就点击 Connect
- **THEN** 系统在 ID 输入框下方显示当前语言的错误提示，不发起连接

#### Scenario: Invalid server URL
- **WHEN** 用户填写的 Server URL 不以 ws:// 或 wss:// 开头
- **THEN** 系统在 Server URL 输入框下方显示当前语言的错误提示，不发起连接

### Requirement: Connection status feedback
系统 SHALL 在连接过程中显示 loading 状态，连接失败时显示错误信息并允许重试。所有状态文本 SHALL 通过 `t()` 函数获取。

#### Scenario: Connection in progress
- **WHEN** 用户点击 Connect 后正在建立连接
- **THEN** Connect 按钮显示当前语言的 loading 状态文本且不可重复点击

#### Scenario: Connection failed
- **WHEN** 连接失败（服务器不可达或拒绝）
- **THEN** 系统显示当前语言的错误信息，Connect 按钮恢复可点击状态
